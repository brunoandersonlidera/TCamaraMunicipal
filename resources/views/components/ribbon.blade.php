@props([
    // Component size (optional). You can set either width/height or control via CSS.
    'width' => '18.8861mm',
    'height' => '35.4142mm',
    // Colors are optional; if omitted, they fallback to theme variables.
    // Use CSS variables or hex (e.g., '#FF6699').
    'fillPrimary' => null, // main ribbon color (e.g., October Pink)
    'fillBase' => null,    // base/white fill
    'stroke' => null,      // outline color
    'class' => '',
    'style' => '',
])
...
<svg
    ...
    height="{{ $height }}"
    class="{{ $class }}"
    style="--ribbon-fill-primary: {{ $fillPrimary ?? 'var(--theme-accent, #FF6699)' }}; --ribbon-fill-base: {{ $fillBase ?? 'var(--theme-light, #FEFEFE)' }}; --ribbon-stroke: {{ $stroke ?? 'var(--theme-light, #FEFEFE)' }}; {{ $style }}"
>
    <defs>
        <style type="text/css">
            .str0 { stroke: var(--ribbon-stroke, #FEFEFE); stroke-width:2.4; stroke-miterlimit:22.9256 }
            .fil0 { fill: var(--ribbon-fill-base, #FEFEFE) }
            .fil1 { fill: var(--ribbon-fill-primary, #FF6699) }
        </style>
    </defs>
    <g id="Camada_x0020_1">
